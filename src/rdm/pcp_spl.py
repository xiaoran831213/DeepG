...
class PcpSmx(Pcp):
    ...
    elements and a bias(or intercept), followed by an entry-wise softmax.
    """
    def __init__(self, dim, **kwd):
        """
        ...
        **b: (optional) bias of output (hidden) units, of dimension Q,
        ...
        """
        super(PcpSmx, self).__init__(**kwd)
    ...
    def __repr__(self):
        return '{}({}x{})'.format('SMX', self.dim[0], self.dim[1])
    ...
    def __expr__(self, x):
        ...
        # the shape, larger means sharper at 0
        if not hlp.is_tshr(self.shp):
            self.shp = C(self.shp, 'Shp')

        # activation happens here
        T.nnet.softplus(_ * self.shp) / self.shp
