...
def main():
    ...
    #     d = d/2
    # d = [d]*4 + [int(d/2**_) for _ in range(1, 5) if 2**_ <= d]
    d = [d] + [32] * 10
    n = SAE.from_dim(d, s=T.nnet.relu)
    n[-1].s = T.nnet.sigmoid
    n[+0].s = T.nnet.sigmoid

    # for _ in n[:-1]:
    #     _.s = softplus
    # t1 = []
    # for i in range(len(d)-1):
    #     t1.append(Tnr(n.sub(i+1, 0), x, u=u, lrt=1e-4))
    t1 = Tnr(n, x, u=u, lrt=1e-4, err='CE', bsz=10)
    t2 = Tnr(n, x, u=u, lrt=1e-4, err='L2', bsz=10)
    return t1, t2
